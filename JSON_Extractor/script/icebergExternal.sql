...
GRANT ALL ON EXTERNAL VOLUME ext_vol_demo_sf_ext_catalog TO ROLE iceberg_lab WITH GRANT OPTION;
...
CREATE OR REPLACE CATALOG INTEGRATION demo_open_catalog_ext 
  ...
  REST_CONFIG = (
    ...
    CATALOG_NAME = 'demo_sf_ext_catalog'
  )
  REST_AUTHENTICATION = (
    TYPE = OAUTH 
...
ALTER DATABASE iceberg_lab SET CATALOG_SYNC = 'demo_open_catalog_ext';  

ALTER ICEBERG TABLE sfm_iceberg_customer REFRESH;


use database iceberg_lab;
create schema if not exists iceberg_lab_ext;
use schema iceberg_lab_ext;

CREATE OR REPLACE ICEBERG TABLE sfm_iceberg_customer (
    c_custkey INTEGER,
    c_name STRING,
    c_address STRING,
    c_nationkey INTEGER,
    ...
    CATALOG='SNOWFLAKE'
    EXTERNAL_VOLUME='ext_vol_demo_sf_ext_catalog'
    BASE_LOCATION='sfm_iceberg_customer';

INSERT INTO sfm_iceberg_customer
  SELECT * FROM snowflake_sample_data.tpch_sf1.customer limit 10000;

SELECT *
FROM sfm_iceberg_customer c
...
SELECT
    ...
    before_row_count
FROM sfm_iceberg_customer
JOIN (
        SELECT count(*) AS before_row_count
        FROM sfm_iceberg_customer BEFORE(statement => LAST_QUERY_ID())
    )
    ON 1=1
GROUP BY 2;    
